...
from .models import Menu, Cliente, Mains
...
def jugador_details(request, pk):
    jugador = get_object_or_404(Menu, pk=pk)
...
def jugador_nuevo(request):
    if request.method == "POST":
        ...
        if formulario.is_valid():
            jugador = Menu.objects.create(nombre=formulario.cleaned_data['nombre'], anio = formulario.cleaned_data['anio'])
            for personaje_id in request.POST.getlist('personajes'):
                mains = Mains(personaje_id=personaje_id, jugador_id = jugador.id)
                mains.save()
            messages.add_message(request, messages.SUCCESS, 'MEnu agregado')
...
def personaje_nuevo(request):
    if request.method == "POST":
        ...
        if formulario.is_valid():
            personaje = Cliente.objects.create(nombre=formulario.cleaned_data['nombre'], anio = formulario.cleaned_data['anio'])
